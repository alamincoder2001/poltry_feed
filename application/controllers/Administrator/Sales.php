...
<?php
    ...
    public function getBatch()
    {
        ...
        $batch = $this->db->query("SELECT 
                                p.Product_SlNo,
                                p.Product_Code,
                                p.Product_Name,
                                pd.Batch_No,
                                sum(pd.PurchaseDetails_TotalQuantity) AS purchase_qty,
                                ifnull((SELECT SUM(sd.SaleDetails_TotalQuantity) 
                                FROM tbl_saledetails sd 
                                WHERE sd.Status = 'a'
                                AND sd.SaleDetails_BranchId = '$this->sbrunch'
                                AND sd.Batch_No = pd.Batch_No
                                AND sd.Product_IDNo = p.Product_SlNo
                                ), 0) as sale_qty,
                                (select pd.PurchaseDetails_TotalQuantity - sale_qty) as stock
                            ...
                            $clauses GROUP BY pd.Batch_No")->result();
